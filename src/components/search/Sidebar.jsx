...
const Sidebar = ({ categoryName, totalResults, query, sbfilters }) => {
    ...
    const envio = sbfilters.find(filter => filter.id === 'shipping')
    const price = sbfilters.find(filter => filter.id === 'price')
    const cuota = sbfilters.find(filter => filter.id === 'installments')
    const descuento = sbfilters.find(filter => filter.id === 'discount')
    const oferta_dia = sbfilters.find(filter => filter.id === 'promotion_type')
    const ubicacion = sbfilters.find(filter => filter.id === 'state')
    const tienda_oficial = sbfilters.find(filter => filter.id === 'official_store')
    const bestseller = sbfilters.find(filter => filter.id === 'power_seller')
    const internacional = sbfilters.find(filter => filter.id === 'SHIPPING_ORIGIN')
            ...
            <aside className="aside-wrapper">
                ...
                {   internacional &&
                    <div className="aside-ubication-filter aside-filter">
                        ...
                        {   internacional.values.map( (item, index) => (
                                <div key={index} className="filter-option">
                                    <a href="#">
                                        <span> {`${internacional.values[index].name}`} </span>
                                        <span> {`(${internacional.values[index].results})`} </span>
                                    </a>
                                </div>
                            ))
                        }
